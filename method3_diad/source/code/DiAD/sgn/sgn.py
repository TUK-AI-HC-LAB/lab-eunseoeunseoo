...
class DiAD(LatentDiffusion):
    ...
    def get_unconditional_conditioning(self, N):
        # return self.get_learned_conditioning({self.cond_stage_key: torch.tensor([15]* N).to(self.device)})
    ...
    @torch.no_grad()
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=10, ddim_eta=0.0, return_keys=None,
                   quantize_denoised=True, inpaint=True, plot_denoise_rows=False, plot_progressive_rows=True,
                   plot_diffusion_rows=False, unconditional_guidance_scale=9.0, unconditional_guidance_label=None,
                   use_ema_scope=True,
                   **kwargs):
