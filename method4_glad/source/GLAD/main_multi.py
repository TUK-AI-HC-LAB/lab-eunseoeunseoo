...
def parse_args(input_args=None):
    ...
    # protocol change: the eval branch below hardcodes checkpoint_step=20000 per
    # dataset (i.e. it can only ever evaluate a checkpoint saved at exactly the
    # final training step). We need to evaluate intermediate checkpoints
    # (200/400/600/...) to track H3/H4 trends the same way DiAD's epoch7/16/34
    # points were tracked. Add an optional override.
    parser.add_argument("--checkpoint_step_override", type=int, default=None)
...
def main(args, class_name):
    ...
    # protocol change: the official script only calls accelerator.save_state() and never
    # load_state() -- a crash/reboot always restarted training from the pretrained SD
    # weights with zero optimizer progress, no matter how often checkpoints were saved.
    # On this hardware, unplanned interruptions (Windows Update forced reboots, thermal/
    # battery cutoffs) are common enough that this made checkpointing_steps pointless.
    # Resume from the highest-step checkpoint-N dir under output_dir if one exists.
    import re
    existing_ckpts = []
    if os.path.isdir(args.output_dir):
        for d in os.listdir(args.output_dir):
            m = re.fullmatch(r"checkpoint-(\d+)", d)
            if m and os.path.isdir(os.path.join(args.output_dir, d)):
                existing_ckpts.append((int(m.group(1)), os.path.join(args.output_dir, d)))
    if existing_ckpts:
        global_step, resume_path = max(existing_ckpts, key=lambda x: x[0])
        accelerator.load_state(resume_path)
        logger.info(f"Resumed from checkpoint {resume_path} at global_step={global_step}")
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        # protocol change: our MVTec-AD copy lives under a folder named
        # diad_dataset (inherited from method3_diad), not 'MVTec-AD', so the
        # original substring check silently fell through and left
        # checkpoint_step undefined (NameError). Accept both names.
        if 'MVTec-AD' in args.instance_data_dir or 'diad_dataset' in args.instance_data_dir:
            args.input_threshold = 0.45
            args.denoise_step = 650
            args.min_step = 350
            args.resolution = 256
            args.dino_resolution = 256
            checkpoint_step = args.checkpoint_step_override if args.checkpoint_step_override else 20000
            args.v = 0
