...
class MVTecDataset(Dataset):
    ...
    def anomaly_source(self, img, anomaly_path_list, aug):
        ...
        if p < 0.5:
            ...
            # protocol change: imgaug==0.4.0 (as pinned in requirements.txt) gates several
            # augmenters in `aug` (Posterize/Solarize/Invert/Equalize etc.) to uint8-only and
            # raises ValueError on float32 input. The other branch below already passes a
            # uint8 array (from PIL); match that here instead of casting to float32 first.
            anomaly_source_img = cv2.resize(texture_source_img, (self.resize, self.resize)).astype(np.uint8)
            anomaly_source_img = aug(image=anomaly_source_img).astype(np.float32)
        else:
