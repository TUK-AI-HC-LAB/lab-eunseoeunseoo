...
def train(item):
    setup_seed(1)
    print_fn(item)
    total_iters = 200  # BENCH: short run for GPU log only
    batch_size = 4  # BENCH: batch=4
    image_size = 448
    crop_size = 392
    ...
    model = ViTill(encoder=encoder, bottleneck=bottleneck, decoder=decoder, target_layers=target_layers,
                   ...
                   use_checkpoint=False)  # BENCH: original (no gradient checkpointing) path, like the first batch=4 run
