...
def train(item):
    setup_seed(1)
    print_fn(item)
    total_iters = 30  # BENCH: short run for GPU log only
    batch_size = 16  # BENCH: batch=16
    image_size = 448
    crop_size = 392
    ...
    model = ViTill(encoder=encoder, bottleneck=bottleneck, decoder=decoder, target_layers=target_layers,
                   ...
                   use_checkpoint=False)  # BENCH: original (no gradient checkpointing) path, the path where batch=16 spilled into shared memory
    ...
    for epoch in range(int(np.ceil(total_iters / len(train_dataloader)))):
        ...
        for img, label in train_dataloader:
            ...
            it += 1
            if it % 2 == 0:
                print_fn('BENCH iter {} t={:.2f}'.format(it, __import__('time').time()))
            if it == total_iters:
                break
